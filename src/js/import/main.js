...
function textTableScrollInit() {
    ...
    const scrollArrowLayout = `<svg>
                                <use xlink:href="${window.distPath}img/sprites/sprite.svg#arrow_classic2"></use>
                              </svg>`
    ...
    tables.forEach(table => {
        if(table.parentElement.closest('table') === null) {
            ...
            if(tabs) {
                tabsListHeight = tabs.querySelector('[data-js="tabsBlockList"]').offsetHeight
                maxHeight = maxHeight - tabsListHeight
                tabsPadding = parseInt(window.getComputedStyle(tabs.parentElement).paddingTop)
            }
            ...
            prev.innerHTML = scrollArrowLayout
            next.innerHTML = scrollArrowLayout
            prev.classList.add('slider-control', 'slider-control--scroll', 'slider-control--prev')
            next.classList.add('slider-control', 'slider-control--scroll', 'slider-control--next')
            ...
            const slide = wrapper.closest('[data-js="tabsBlockSlide"]')
            
            slide.addEventListener('activatedSlide', function() {
                
                prev.style.top = tabsListHeight + 10 + tabsPadding + "px";
                next.style.top = tabsListHeight + 10 + tabsPadding + "px";
                
                const maxScroll = wrapper.scrollWidth - wrapper.offsetWidth
                const isScrollable = maxScroll > 0 ? true : false
...
function referencesController() {
    ...
    ymaps.ready(async function () {
        ...
        // получаем все точки по миру
        allWorldCities = getPoints()
    ...
    function tabsOnChange() {
        ...
        if(currentType == 'country') {
            ...
            currentCities = JSON.parse(JSON.stringify(getPoints()))
        }
